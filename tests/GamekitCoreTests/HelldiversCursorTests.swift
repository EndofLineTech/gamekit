...
struct HelldiversCursorTests {
    @Test("Query or change only the game's cursor-confinement override while the prefix is idle",
    ...
    func configure() async throws {
        ...
        try #require(["query", "event-tap", "restore-default"].contains(action))
        ...
        do {
            ...
            if let expected = env["GAMEKIT_CURSOR_EXPECT"] {
                try #require(["absent", "event-tap", "existing-other"].contains(expected))
                #expect(result.stdoutText.contains("cursor_override=" + expected))
            }
