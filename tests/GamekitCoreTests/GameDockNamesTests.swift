...
struct GameDockNamesTests {
    ...
    func mapping() throws {
        ...
        try GameDockNames.publish(root: store.root, prefix: prefix, session: first, games: [game], graphicsBackend: .metal3, validate: {})
        ...
        #expect(map.sharedGraphicsBackend == .automatic && map.graphicsBackends?.isEmpty == true)
    }
