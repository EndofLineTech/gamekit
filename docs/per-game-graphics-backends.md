...
DXVK and DXMT remain disabled **In Dev** entries. They are not implemented by
these overrides. The panel shows both the shared default and the effective
...
Setup continues to choose the backend for Windows Steam and games that inherit
it. Changing a game's dropdown does not change `RuntimeSelection.json`, another
...
Values are `automatic` or `metal3`; absent entries inherit. Schema-1 driver
...
No game files, registry values or graphics DLLs are modified for backend choices.
...
For non-game images the helper restores the shared backend, preventing an
...
- UI tests verified a non-Helldivers game's dropdown, disabled development
  entries, persistence across app restart, unchanged shared backend and restored
